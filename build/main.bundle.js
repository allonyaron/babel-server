...
rh.addons = function () {
    ...
    switch (nydn.template) {
        case "article":
        case "ymm":
            rh.download(nydn.urls.ra);
            break;
        case "homepage":
        case "index":
            rh.download(nydn.urls.rhp);
            break;
        case "ss-hub":
        case "ss-category":
            rh.download(nydn.urls.rss);
            break;
        case "author":
            rh.download(nydn.urls.author);
            break;
        case "search":
            rh.download(nydn.urls.search);
        case "tag":
            rh.download(nydn.urls.tag);
            break;
        case "photos":
            rh.download(nydn.urls.photos);
            break;
        default:
...
rh.bp.initialize = function () {
    ...
    if (rh.bp.size < rh.bp.tablet) rh.platform = "mobile";else if (rh.bp.size < rh.bp.desktop) rh.platform = "tablet";else rh.platform = "desktop";
    ...
    rh.script.download = function (scriptURL, scriptObj) {
        (function () {
            ...
            if (!rhScript.downloaded) {
                //if not appended to a custom parent node
...
//CLICK
{
    rh.click = function () {
        $("#rh").on("click", function (e) {
            if (e.target.id != "") rh.target = e.target.id;else if ($(e.target).attr("class") != "" && $(e.target).attr("class") != undefined) {
...
//FOLLOW
{
    ...
    rh.follow.toggle = function () {
        if (!$("#rh-follow").hasClass("on")) $("#rh-follow").addClass("on");else $("#rh-follow.on").removeClass("on");
    };
            ...
            $(window).resize(function () {
                ...
                resizeTimeout = setTimeout(function () {
                    if ($(window).width() != windowWidth) {
                        ...
                        var changeBP = false;
                        if ((startPlatform === 'mobile' || startPlatform === 'tablet') && (endPlatform === 'mobile' || endPlatform === 'tablet')) changeBP = false;else if (startPlatform === 'desktop' && endPlatform === 'desktop') changeBP = false;else changeBP = true;
...
//UTILITY
{
    ...
    rh.utility.afterAnimation = function (nydnQueryElement, callback, elementToAnimate) {
        nydnQueryElement.one('webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend', function (e) {
            // console.dir(e);
...
//STICKY
{
    ...
    rh.stickyNav.initialize = function () {
        ...
        if ($(".rh-ad").length && $('#rh-subnav').is(':visible')) rh.stickyNav.top = $("#rh-masthead").position().top + $("#rh-masthead").outerHeight(true) + $(".rh-ad").outerHeight(true);else if ($('#rh-subnav').is(':visible')) rh.stickyNav.top = $("#rh-subnav").position().top + $("#rh-subnav").outerHeight(true);else rh.stickyNav.top = $("#rh-masthead").position().top + $("#rh-masthead").outerHeight(true);
    ...
    rh.stickyAd.stickAd = function () {
        ...
        $(window).on("scroll.adScroll", function () {
            if ($(window).scrollTop() > 0) $(".rh-ad").addClass("adScrolled");else $(".rh-ad").removeClass("adScrolled");
        });
        rh.stickyAd.addOffset();
    };
...
//NEWSROOM
{
    ...
    rh.newsroom.initialize = function () {
        ...
        if (nydn.section === "entertainment") {
            window._newsroom = window._newsroom || [];
            window._newsroom.push({ pageTemplate: 'Entertainment' });
            window._newsroom.push({ pageDashboard: 'Entertainment' });
            window._newsroom.push('auditClicks');
            window._newsroom.push('trackPage');
        }
...
//GTM
{
    ...
    rh.gtm.getURL = function () {
        if (window.location.href.indexOf("wwwqa") > -1 || window.location.href.indexOf("interactiveqa") > -1) rh.gtm.url = 'GTM-MFQKX7';else rh.gtm.url = 'GTM-M9G2T8';
...
}
{
    ...
    Slider.prototype.hoverOn = function (e) {
        if (this.start) this.right.addClass('on');else if (this.end) this.left.addClass('on');else {
    ...
    Slider.prototype.next = function () {
        ...
        return function (e) {
            ...
            if (!that.end) {
                ...
                that.container.animate({ scrollLeft: that.scrollingTo }, 300, function () {
                    that.endCheck();
                });
            }
        };
    ...
    Slider.prototype.endCheck = function () {
        ...
        if (this.container.is(':animated')) {
            ...
            if (availableSpace <= 2) this.end = true;else this.end = false;
            ...
            if (this.container.scrollLeft() <= 0) this.start = true;else this.start = false;
            this.hoverOff();
            this.hoverOn();
        }
    };
    Slider.prototype.autoAdvance = function (duration) {
        ...
        that.autoTimer = setInterval(function () {
            ...
            if (that.end) {
                ...
                that.container.animate({ scrollLeft: 0 }, 1000, function () {
                    that.endCheck();
                });
            }
        }, duration);
...
//GLOBAL SHARE
var rShare = function rShare(shNode, shOptions) {
    var this_ = this;
    nydnRequires([nydn.urls.nydnQuery], function (util) {
...
};
{
    ...
    rShare.prototype.initialize = function () {
        ...
        if (!this.initialized) {
            if (isTouchDevice()) this.touching();else this.mousing();
        }
    };
    ...
    rShare.prototype.getBitly = function () {
        ...
        var this_ = this;
        nydnRequires([nydn.urls.nydnQuery], function (util) {
            $.getJSON("http://api.bitly.com/v3/shorten?callback=?", { "format": "json",
    ...
    rShare.prototype.getInfo = function () {
        ...
        } else if (document.querySelectorAll(".rc.bd").length) {
...
//AOL
{
    ...
    rh.getAolVars = function () {
        ...
        if (!!document.querySelector("[id*='r-aol-video']")) {
            var aolVideoDiv = document.querySelector("[id*='r-aol-video']");
            rh.aol.vars.aolVideoDivId = aolVideoDiv.id + '-' + aolVideoDiv.dataset.videoId;
        };
    };
    rh.aol.params = function () {
        ...
        "m.output=" + rh.aol.vars.output + "&m.iu=" + rh.aol.vars.iu + "&m.sz=" + rh.aol.vars.sz + "&m.env=" + rh.aol.vars.env + "&m.gdfp_req=" + rh.aol.vars.gdfp_req + "&m.impl=" + rh.aol.vars.impl + "&m.unviewed_position_start=" + rh.aol.vars.unviewed_position_start + "&m.Content=" + rh.aol.vars.Content + "&m.ContentID=" + rh.aol.vars.ContentID + "&m.Package=" + rh.aol.vars.Package + "&m.nsfa=" + rh.aol.vars.nsfa + "&m.qa=" + rh.aol.vars.qa + "&m.testAd=" + rh.aol.vars.testAd + "&m.viewport=" + rh.aol.vars.viewport + "&m.videoID=" + rh.aol.vars.videoID + "&m.playerID=" + rh.aol.vars.playerID + "&m.videoPlaylistID=" + rh.aol.vars.videoPlaylistID + "&m.url=" + rh.aol.vars.url + "&m.description_url=" + rh.aol.vars.description_url + "&m.correlator=" + rh.aol.vars.timestamp + "&m.oview=oVa[" + rh.aol.vars.aolVideoDivId + "];";
    ...
    rh.aol.getPlaylistID = function () {
        var videoPlaylistObj = rh.aol.tags.getThisObject("section", rh.aol.vars.section.toLowerCase()),
            //get playlist obj  {"entertiament":"abcdefgID"}
    ...
    rh.aol.download = function () {
        ...
        if (!!document.getElementById("r-aol-video")) {
            var vNodes = document.getElementsByClassName("vdb_player"),
                //get all AOL video divs
            ...
            for (var i = 0; i < vNodes.length; i++) {
                ...
                if (nydn.template == "article") {
                    if (i || nydn.reskin) thisPlayerID = "58d0493bf78ced6518eb88fa";else thisPlayerID = "5852e8f9cc52c7184f640758";
                } else {
                ...
                if (nydn.template == "article") rh.aol.url = thisVideoPlaylistID != "" && thisVideoPlaylistID != undefined ? "//delivery.vidible.tv/jsonp/bid=" + thisVideoPlaylistID + "/pid=" + thisPlayerID + "/vid=" + thisVideoID + "/" + rh.aol.vars.BuyerCompanyID + ".js?" + thisParams : "//delivery.vidible.tv/jsonp/pid=" + thisPlayerID + "/vid=" + thisVideoID + "/" + rh.aol.vars.BuyerCompanyID + ".js?" + thisParams;else rh.aol.url = thisVideoPlaylistID != "" && thisVideoPlaylistID != undefined ? "//delivery.vidible.tv/jsonp/bid=" + thisVideoPlaylistID + "/pid=" + thisPlayerID + "/" + rh.aol.vars.BuyerCompanyID + ".js?" + thisParams : "//delivery.vidible.tv/jsonp/pid=" + thisPlayerID + "/" + rh.aol.vars.BuyerCompanyID + ".js?" + thisParams;
...
//LOTAME ANALYTICS
{
    ...
    rh.lotame.global.exe = function () {
        if (document.readyState === "complete") _cc4866.bcpf();else _cc4866.bcp();
    ...
    rh.lotame.autos.exe = function () {
        if (document.readyState === "complete") _cc6968.bcpf();else _cc6968.bcp();
...
var rhAdArr2 = [];
{
    ...
    rh.ads.dependencies.download = function () {
        ...
        if (!rh.ads.dependencies.downloaded) nydnRequires(rh.ads.dfp.waitingList, function (util) {
...
//DFP
{
    ...
    rh.ads.dfp.defineAd = function (rhpAdTag, rhpAdX, rhpAdSizeArray) {
        ...
        if (!!document.getElementById(rhpAdX) && !!rhpAdSizeArray) {
            switch (rhpAdX) {
                case "adBrandPov":
                    rhpAdTag = googletag.defineSlot(rh.ads.dfp.defineSlotStr, rhpAdSizeArray, rhpAdX).addService(googletag.pubads()).setTargeting("ntve", "bpov").setTargeting("position", "adBrandPov").setTargeting("Conpos", nydn.content + "-" + "adBrandPov");
                    break;
                case "div-gpt-ad-x100":
                    rhpAdTag = googletag.defineOutOfPageSlot(rh.ads.dfp.defineSlotStr, rhpAdX).addService(googletag.pubads()).setTargeting("Conpos", nydn.content + "-" + rhpAdX.slice(11)).setTargeting("position", rhpAdX.slice(11));
                    break;
                default:
                    ...
                    rhpAdTag = googletag.defineSlot(rh.ads.dfp.defineSlotStr, rhpAdSizeArray, rhpAdX).addService(googletag.pubads()).setTargeting("Conpos", nydn.content + "-" + rhpAdX.slice(11)).setTargeting("position", rhpAdX.slice(11));
            }
    ...
    rh.ads.dfp.contentstudio = function (rhpAdX, rhpAdSizeArray) {
        if (nydn.section == "contentstudio" && nydn.template == "ss-category" && rh.platform != "mobile" && (rhpAdX == "div-gpt-ad-x102" || rhpAdX == "div-gpt-ad-x103" || rhpAdX == "div-gpt-ad-x104")) rhpAdSizeArray = rh.adsData.box1[rh.platform]["ct-ss-category"];
    ...
    rh.ads.dfp.mobile = function () {
        ...
        googletag.cmd.push(function () {
            ...
            rh.adsWide1 = rh.ads.dfp.defineAd(rh.adsWide1, rh.adsData.wide1.x, rh.adsData.wide1[rh.platform].oneSize);
            if (nydn.template == "ss-category" || nydn.template == "ss-hub") rh.adsBox1 = rh.ads.dfp.defineAd(rh.adsBox1, rh.adsData.box1.x, rh.adsData.box1[rh.platform].oneSize);
            rh.adsOut2 = rh.ads.dfp.defineAd(rh.adsOut2, rh.adsData.out2.x, rh.adsData.out2.oneSize);
    ...
    rh.ads.dfp.targeting = function () {
        ...
        googletag.pubads().setTargeting("Content", nydn.content).setTargeting("Package", nydn.packageName).setTargeting("ContentID", nydn.contentID).setTargeting("video", rh.ads.dfp.video).setTargeting("nsfa", nydn.nsfa).setTargeting("testAd", nydn.testAd).setTargeting("flexKey", nydn.flexKey).setTargeting("viewport", rh.platform).setTargeting("page_url", currentLocation);
        ...
        if (nydn.hasOwnProperty("flexName") && nydn.hasOwnProperty("flexValue")) googletag.pubads().setTargeting(nydn.flexName, nydn.flexValue);
    ...
    rh.ads.dfp.moveinContent = function () {
        ...
        switch (nydn.template) {
            case "homepage":
                rh.ads.dfp.moveinContentHP();break;
            case "index":
                rh.ads.dfp.moveinContentIndex();break;
            case "tag":
            case "search":
            case "ymm":
            case "article":
                rh.ads.dfp.moveinContentArticle();break;
            case "ss-hub":
                rh.ads.dfp.moveinContentHub();break;
        }
    };
    ...
    rh.ads.dfp.refresh = function () {
        if (!rh.ads.dfp.refreshed) nydnRequires(rh.ads.dfp.waitingList, function (util) {
            ...
            rhAdArr2.forEach(function (rhAdx) {
                if (rhAdx !== "div-gpt-ad-x100" && rhAdx != "div-gpt-ad-x108") {
                    switch (rhAdx) {
                        ...
                        case "div-gpt-ad-x101":
                            sonobiID = "b2d486d7815142a5d9b3";break;
                        case "div-gpt-ad-x102":
                            sonobiID = "ebfb261c83ba67a16a8e";break;
                        case "div-gpt-ad-x103":
                            sonobiID = "2f6922a90e5df1d3a720";break;
                        case "div-gpt-ad-x104":
                            sonobiID = "549ce0805e7b05cad523";break;
                        case "div-gpt-ad-x105":
                            sonobiID = "3372e220441848388d87";break;
                    }
...
//OPENX LITE
{
    ...
    rh.ads.openx.lite.initialize = function () {
        nydnRequires([rh.ads.openx.lite.url], function (util) {
            ...
            var oxAdMappings = [];
            rhAdArr2.forEach(function (rhAdx) {
                if (rh.ads.openx.lite.adUnitIDs[rhAdx] != "") oxAdMappings.push({
                    type: 'display',
                    divId: rhAdx,
...
var oVa = oVa || {};
{
    ...
    rh.ads.optimera.getURL = function () {
        //the JSON URL that contains the oVa values. It doesn't exist for new articles.
    ...
    rh.ads.optimera.setup = function () {
        //set up defauly valuse for oDv and oVa BEFORE downloading JSON
    ...
    rh.ads.optimera.download = function () {
        //download JSON with final oVa
    ...
    rh.ads.optimera.download2 = function () {
        //download oPs that reports ad units loading
        nydnRequires([rh.ads.optimera.url], function (util) {
            console.log("nydn 🎯  rh.ads.optimera.download 2 ✓");
        });
    };
    rh.ads.optimera.targeting = function () {
        //loops thourgh defined ad nits, and setTargeting
        var rhAdx;
        nydnRequires([nydn.urls.optimera], function (util) {
            //must wait for JSON if it exists.
...
//YIELDMO
{
    ...
    rh.ads.yieldmo.initialize = function () {
        ...
        if (rh.platform === "mobile" && !nydn.bidder.contains("jumpstart")) {
            ...
            if (nydn.section == "nydailynews" || nydn.section == "homepage" || nydn.section == "home") yieldmoSection = "homepage";else if (nydn.subsection == "horoscopes") yieldmoSection = nydn.subsection;else if (nydn.section == "news" || nydn.section == "new york") yieldmoSection = nydn.section + " subsection";
            ...
            if (yieldmoTag.getThisObject("ymS", yieldmoSection)) rh.ads.yieldmo.exe(yieldmoTag.getThisObject("ymS", yieldmoSection));else rh.ads.yieldmo.exe(yieldmoTag.getThisObject("ymS", "homepage"));
    ...
    rh.ads.yieldmo.exe = function (inContentID) {
        ...
        ymDivs.each(function (i) {
            ymC = i + 1;
            if (ymDivsL == ymC) $(this).attr("id", inContentID["ymIDf"]);else if (i < ymDivsL) $(this).attr("id", inContentID["ymID" + ymC]);
        });
    };
    ...
    rh.ads.yieldmo.download = function () {
        (function (e, t) {
            if (t._ym === void 0) {
                ...
                m.type = "text/javascript", m.async = !0, m.src = "//static.yieldmo.com/ym.m1.js", (e.getElementsByTagName("head")[0] || e.getElementsByTagName("body")[0]).appendChild(m);
...
requirejs.onError = function (err) {
    ...
    if (rh.ads.dfp.waitingList.contains(err.requireModules)) {
        ...
        if (!rh.ads.dependencies.downloaded) rh.ads.dependencies.download();else if (!rh.ads.dfp.refreshed) rh.ads.dfp.refresh();
    }
